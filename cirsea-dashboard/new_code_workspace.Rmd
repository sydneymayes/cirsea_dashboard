...
platform_range_vec[5] <- platform_range_vec[5]*2 # excludes process for smart buoy
platform_range_vec[8] <- platform_range_vec[8]*2 # excludes process for onshore command center
platform_range_vec[8] <- platform_range_vec[9]*2 # excludes process for onboard
platform_range_vec
...
# fix the bin ranges based on new data -- UPDATE A SAVED VERSION OF THIS FILE IF CHANGES ARE MADE -------------
...
sensor_platform_combinations$sensor_platform <- with(sensor_platform_combinations, paste(sensor, "by", platform))

# going to save this and manually add data_type by sensor (WRITE CODE LATER)
# write_csv(sensor_platform_combinations, "./data/sensor_platform_combinations_data_type.csv")

sensor_platform_combinations_df <- read_csv("data/sensor_platform_combinations_data_type.csv")
...
index_tidy <- merged_index_df %>%
  ...
  pivot_longer(cols = starts_with("long_range_camera by small_usv"):ends_with("observer by onboard"), 
...
index_tidy_more <- index_tidy %>%
  left_join(sensor_platform_combinations_df %>% 
              select(sensor_platform, total_cost, data_type),
            by = "sensor_platform")
